...
async def download_all_sgc_files(user: User = Depends(get_sgc_user)):
    """
    Download a pre-built ZIP archive of SGC cohort files.
    Requires 'sgc-review-data' permission.
    The archive is generated by a scheduled job (scripts/build_sgc_archive.py).
    GWAS summary-stat files are not included; fetch them individually via the SGC UI.
...
# Deletion is handled by deleting the parent cohort (CASCADE).
# ---------------------------------------------------------------------------

@router.get("/sgc/cohorts/{cohort_id}/gwas-metadata")
...
@router.post("/sgc/cohorts/{cohort_id}/gwas-metadata")
async def create_sgc_gwas_metadata(cohort_id: str, cohort: SGCGWASCohort, user: User = Depends(get_sgc_user)):
    from sqlalchemy.exc import IntegrityError
    try:
        cohort.submitted_by = user.user_name
        cohort.cohort_id = cohort_id
        record_id = query.insert_sgc_gwas_cohort(engine, cohort)
        return {"message": "GWAS metadata saved successfully", "id": record_id}
    except IntegrityError:
        raise fastapi.HTTPException(status_code=409, detail="GWAS metadata already exists for this cohort")
    except Exception as e:
        raise fastapi.HTTPException(status_code=500, detail=f"Error saving GWAS metadata: {str(e)}")


@router.put("/sgc/cohorts/{cohort_id}/gwas-metadata")
async def update_sgc_gwas_metadata(cohort_id: str, cohort: SGCGWASCohort, user: User = Depends(get_sgc_user)):
    existing = query.get_sgc_gwas_cohort_by_cohort_id(engine, cohort_id)
    if not existing:
