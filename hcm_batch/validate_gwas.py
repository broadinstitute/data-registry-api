#!/usr/bin/env python3
"""
HCM GWAS row-level validation batch job.

Downloads a GWAS file from S3, validates every data row against the column
mapping rules from the HCM GWAS analysis plan (Section 9.1), and writes
periodic progress updates to an S3 JSON file so the API can report
percent-complete.

Validation rules (per mapped column):
  variant_id              : non-empty string (required)
  chromosome              : 1-22 or "X" (required)
  position                : positive integer (required)
  effect_allele           : non-empty, chars in {A,C,T,G,-,N} (required)
  non_effect_allele       : same as effect_allele (required)
...
# Keys match HCM_TARGET_FIELDS from the API.
# Cases and controls are optional per Section 9.1 ("where available").
FIELD_VALIDATORS = {
    "variant_id":              (validate_variant_id, True),
    "chromosome":              (validate_chromosome, True),
    "position":                (validate_position, True),
    "effect_allele":           (validate_allele, True),
    "non_effect_allele":       (validate_allele, True),
    "beta":                    (validate_beta, True),
    "standard_error":          (validate_se, True),
    "p_value":                 (validate_pvalue, True),
    "effect_allele_frequency": (validate_eaf, True),
    "sample_size":             (validate_positive_integer, True),
    "imputation_quality":      (validate_info, True),
    "cases":                   (validate_positive_integer, False),
    "controls":                (validate_positive_integer, False),
}
...
def validate_file(file_path: str, column_mapping: dict, tracker: ProgressTracker):
    ...
    with opener(file_path, "rt") as fh:
        ...
        # Build lookup: file_header_name -> (validator_func, mapping_key, is_required)
        validators = {}
        for mapping_key, header_name in column_mapping.items():
            if mapping_key in FIELD_VALIDATORS and header_name in reader.fieldnames:
                validator_fn, required = FIELD_VALIDATORS[mapping_key]
                validators[header_name] = (validator_fn, mapping_key, required)

        for row_idx, row in enumerate(reader, start=2):  # row 1 is header
            for header_name, (validator_fn, mapping_key, required) in validators.items():
