#!/usr/bin/env python3
"""
Reverse a replace-in-place SGC liftover: copy the archived original back over the
...
sgc_liftover_jobs.original_s3_path -- so this restore is a clean, byte-identical undo.
...
string explicitly if you captured it and want an exact restore. (The build lives in
metadata.$.genome_build for SGC files -- the reused set_sgc_gwas_file_build fn
handles that JSON write.)
...
Usage:
  python scripts/restore_sgc_liftover.py --file-id <uuid> --bucket dig-data-registry [--to-build GRCh37] [--dry-run]
"""
import os
import sys
...
_FILE_TABLE = "sgc_gwas_files"
_JOB_TABLE = "sgc_liftover_jobs"
...
def restore(engine, s3_client, file_id, bucket, to_build="GRCh37", dry_run=False) -> dict:
    ...
    query.set_sgc_gwas_file_build(engine, file_id, to_build)
    _delete_liftover_jobs_for_file(engine, file_id)
    return plan


@click.command()
@click.option("--file-id", required=True, help="SGC GWAS file id (uuid) to restore")
